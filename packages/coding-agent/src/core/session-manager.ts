...
export interface CompactionEntry<T = unknown> extends SessionEntryBase {
	...
	fromHook?: boolean;
	/** LLM usage from the summarization call(s). Absent on older sessions. */
...
function hasUserOrAssistantMessage(entries: FileEntry[]): boolean {
	return entries.some(
		(entry) =>
...
export class SessionManager {
	...
	/**
	 * Persist header + in-memory entries if the session already has a user
	 * (or assistant) message and has not been written yet. Empty sessions
	 * (header only) stay unflushed so `/new` does not create empty jsonl files.
	 * Returns true when a file was created by this call.
	 */
	flushIfHasUserMessages(): boolean {
		if (!this.persist || !this.sessionFile || this.flushed) return false;
		if (!hasUserOrAssistantMessage(this.fileEntries)) return false;
		this._rewriteFile();
		this.flushed = true;
		return true;
	}
	...
	_persist(entry: SessionEntry): void {
		...
		const hasContent = hasUserOrAssistantMessage(this.fileEntries);
		if (!hasContent) {
			if (this.flushed) {
				appendFileSync(this.sessionFile, `${JSON.stringify(entry)}\n`);
			} else {
				// Mark as not flushed so when a user/assistant arrives, all entries get written
				this.flushed = false;
			}
			return;
		}
	...
	createBranchedSession(leafId: string): string | undefined {
		...
		if (this.persist) {
			...
			// Write immediately when the branched path already has a user or
			// assistant message. Empty (header-only) paths stay unflushed so
			// they match newSession() and do not create empty jsonl files.
			// Creating the file here also sets flushed, so a later _persist()
			// appends instead of openSync(..., "wx") which would EEXIST.
			if (hasUserOrAssistantMessage(this.fileEntries)) {
				this._rewriteFile();
				this.flushed = true;
			} else {
				this.flushed = false;
