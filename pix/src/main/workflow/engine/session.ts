 ...
 * and posts the run's terminal result exactly once. Keeping it separate from
 * `worker.ts` lets unit tests drive the session over an in-process
 * MessageChannel, because main-process coverage cannot observe code inside a
 * real Worker.
 *
 * The session announces ready and waits for `go`, so cancellation racing
...
import type { HostToWorkerMessage, WorkerToHostPayloads } from "./protocol.js";
...
class ChildRpcBridge implements ChildPort {
  ...
  async startAgent(request: ChildStartRequest): Promise<ChildHandle> {
    ...
    // Containment: when asynchronous provider start fails (or the run is
    // torn down), the settled promise may never gain a consumer — it must
    // not surface as an unhandled rejection and kill the worker.
    entry.settled.promise.catch(() => {
      /* consumed: unconsumed child settlement after failed start */
    });
 ...
 * ultimately terminates the thread). It never rejects: constructor failure
 * becomes an error result. Host pre-parse makes syntax failure here a likely
 * Node-version skew, but the session still reports it instead of dying
 * silently.
