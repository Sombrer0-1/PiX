/**
 * Materializes values leaving the script vm into plain host JSON before they
 * cross the worker boundary, and renders thrown script values without
 * rejecting the run. The walk rejects values that JSON cannot preserve but
 * trusts model-written workflow scripts: getters and proxy traps may run, and
 * the vm is not a security boundary. The worker provides host-loop isolation
 * and forced termination, not hostile-value containment.
 */
...
export function renderThrown(error: unknown): string {
  try {
    ...
    // total (the run never rejects), so fall back to a fixed label.
 ...
 * `Object.prototype` — which we cannot compare by identity across realms). A
 * `Date`/`Map`/class instance has a longer chain and is rejected.
 */
...
function materializeArray(
  ...
  // CANONICAL (String(Number(key)) === key): "01" / "1e0" / "0x1" normalize
  // to in-range integers but are not canonical array indices, so JSON would
